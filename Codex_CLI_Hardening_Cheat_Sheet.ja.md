...
**まず既定の `cached` が何をしているかを押さえます。** 索引済みの結果を返すだけで、ライブページを取りに行きません（実装上も `external_web_access = false`）。任意のページを開かせる経路が閉じるので、そこを通した持ち出しや、取得の瞬間に狙った内容を返す攻撃は届きません。
...
**そして、既定のままでは自分で取りに行く経路がありません。** `network_access` の既定は `false`、`web_search` の既定は `cached` です。前者でコマンドの通信が閉じ、後者でライブ取得が無くなるので、**両方とも触っていない状態のモデルが見られるのは、ワークスペースの中と、索引に既に載っているものだけ**になります。
...
**開発でコードを触らせるなら `indexed` を勧めます。** ページの中身が取れるので調べ物が成立します。既定の `cached` だと調べ物の途中で行き止まりに当たり、そこで慌てて `live` へ飛ぶことになります。**作業中に緩めることになる設定を既定に据えない**、というのがこの文書の姿勢です。
...
| 姿勢 | 設定 | 何のために |
|---|---|---|
| 開発でコードを触らせる | `indexed` | 調べ物は成立させ、値をパラメータに載せた持ち出しを止める |
| 索引に無いものを読ませる必要がある | `live` | 縛りを外すと分かったうえで外す |
| 検索はさせるが、サイトへページを取りに行かせない | `cached` | 持ち出しだけは避けたく、流入は他の手段で受け止める |
| 外部テキストを一切読ませない | `disabled` | ツール定義自体が作られない。手元で閉まる唯一の段 |
