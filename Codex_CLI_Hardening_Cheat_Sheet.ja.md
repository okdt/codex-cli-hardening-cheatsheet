...
5. **ログ** — ここはエンタープライズ現場や、この仕組み自体のデバッグで必要になるかもしれません。Codex CLI は内部に本格的な OpenTelemetry 統合を持っており、ツールの承認/拒否（`codex.tool_decision`）、実行結果、API 呼び出しまでトレース可能です。セッション rollout（`~/.codex/sessions/` 以下の JSONL）も history とは独立して自動保存されます。正直なところ、ソースコードを読むと想像以上に充実しています。
