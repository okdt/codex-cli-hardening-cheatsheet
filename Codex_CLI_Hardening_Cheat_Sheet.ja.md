...
- `indexed`: 外部アクセスを許したうえで、検索インデックス経由に限定する（`cached` より緩い）
- `live`: その場で実ページを取りに行く（`--search` と同じ）

**デフォルトの `cached` は、そのまま使ってよい設定です。** 索引済みの結果を返すだけで、ライブページを取りに行きません（実装上も `external_web_access = false`）。任意のページを開かせる経路が閉じるので、そこを通した持ち出しや、取得の瞬間に狙った内容を返す攻撃は届きません。

**ただし、これをインジェクション対策として数えないでください。** 検索結果に仕込まれたテキストは、索引経由でそのまま届きます。索引は検査ではありません。公式も「任意のライブコンテンツからの露出を減らす」と限定したうえで、結果は信頼できない入力として扱うようにと書いています。閉じているのは取りに行く側であって、入ってくる側ではありません。
...
sandbox_mode = "workspace-write"
allow_login_shell = false
web_search = "cached"          # デフォルト。索引済みの結果を返し、ライブページを取りに行かない
...
| 外部テキストの取り込み | `web_search = "disabled"`（`"indexed"` は外部アクセスを開くので、締める用途では選ばない） | 調べ物ができなくなる |
