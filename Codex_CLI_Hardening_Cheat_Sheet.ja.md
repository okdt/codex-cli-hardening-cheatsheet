...
- **間接的な prompt injection で意図しない行動を取る**
  OWASP `LLM01:2025 Prompt Injection`。たとえば取得した README、Issue、ドキュメント、生成物の中に埋め込まれた指示に引っ張られ、想定外のコマンドや参照を始めることがあります。
- **権限が広すぎて、被害がそのまま拡大する**
  OWASP `LLM06:2025 Excessive Agency`。`danger-full-access` や広い writable root を与えると、誤った判断がそのまま強い操作になります。
- **履歴や文脈に機密情報が残る**
  OWASP `LLM02:2025 Sensitive Information Disclosure`。プロンプト、差分説明、接続先、内部 URL、トークン断片などが端末ローカルに残る可能性があります。
- **もっともらしい提案を過信する**
  OWASP `LLM09:2025 Misinformation`。モデルの提案や設定解釈が正しそうに見えても、検証なしで適用すると危険です。
...
それぞれ意図やスコープは違いますが、共通して言えるのは、これらは「文脈や作業方針を与えるためのファイル」であり、強制力のあるセキュリティ境界ではないということです。
...
- 特に `remote_enabled` を使ったあとに次も同じ気分で作業すると、想定より広い権限で進めてしまうことがあります
