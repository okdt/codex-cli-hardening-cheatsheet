...
**そして、既定のままでは新しい情報に届く経路がありません。** `network_access` の既定は `false`、`web_search` の既定は `cached` です。前者でコマンドの通信が閉じ、後者でライブ取得が無くなるので、**両方とも触っていない状態のモデルが見られるのは、ワークスペースの中と、索引に既に載っているものだけ**になります。「Web 検索が使えるのだから最新の情報を調べられるはず」と考えると外れます。
...
**`indexed` は内部の情報をパラメータに載せて外へ持ち出すシナリオ排除します。** 仕込まれた悪意のある指示の仕上げは、たいていこの形をとるからです。
